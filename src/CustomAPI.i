...
PyObject *convert_buffer_to_numpy_array(const ImageBuffer<T> &image, const NPY_TYPES type) {
 ...
 void call_python_callback(
    PyObject *callback, PyObject *t, PyObject *gray_array,
    PyObject *x_array, PyObject *y_array, PyObject *z_array) {
  PyObject *args = PyTuple_Pack(5, t, gray_array, x_array, y_array, z_array);
...
void parse_images(royale_depth_data *info) {
  ...
  if (G_PYTHON_DATA_CALLBACK) {
    ...
    //PyObject *t = convert_buffer_to_numpy_array(G_T_IMAGE_BUFFER, NPY_INT64);
    int dims[1];
    dims[0] = 2;
    long long timeStamp[2];
    timeStamp[0] = info->timestamp;
    PyObject *t = PyArray_SimpleNewFromData(1,dims,NPY_LONGLONG,timeStamp); //info->timestamp);
    call_python_callback(G_PYTHON_DATA_CALLBACK,t,gray, x, y, z);
